...
class LoginWindow:
    def __init__(self, root):
        ...
        self.credentials = {"HR 26 CH 3604", "MH02CT2727", "DL 3 CAY 9324", "TN66U8215","user", "vKA51MJ8156"}

        self.username_label = ttk.Label(root, text="Identity Number:")
    ...
    def login(self):
        ...
        if username in self.credentials:
            self.root.destroy()  # Close login window if login successful
            root.deiconify()  # Show main application window
        else:
...
class ObjectDetectionApp:
    def __init__(self, root):
        ...
        self.root.withdraw()  # Hide main application window initially

        self.login_window = tk.Toplevel(root)  # Create login window
        self.login = LoginWindow(self.login_window)
        ...
        self.number_plate_button = tk.Button(self.tab1, text="Detect Number Plate", command=self.detect_number_plate)
        self.number_plate_button.pack(pady=10)
    ...
    def detect_number_plate(self):
        # Assuming you have the number plate detection code in a function called plate_show()
        # Call your number plate detection function here
        detect.plate_show()
