...
def getcount():
    dir_path = 'C:/Users/Admin/PycharmProjects/anpr/parking slot/Indian Number Plates'  # Replace this with the path to your directory
    file_count = len([f for f in os.listdir(dir_path) if os.path.isfile(os.path.join(dir_path, f))]) + 1
    print(file_count)
    return file_count
file_count = getcount()
file_name = ''
...
# Define the key to trigger image capture
key_code = ord('c')  # Change this to the desired key code

# Loop until the user presses the key to quit
while True:
    ...
    # If the user pressed the key to capture an image
    if key == key_code:
        # Save the current frame as an image file

        file_name = 'C:/Users/Admin/PycharmProjects/anpr/parking slot/Indian Number Plates/licensed_car'  # Change this to the desired file name
        file_name = file_name + str(file_count) + '.jpeg'
    ...
    # If the user pressed the key to quit
    #elif key == ord('q'):  # Change this to the desired key code
